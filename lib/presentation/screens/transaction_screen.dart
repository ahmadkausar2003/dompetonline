...
class _TransactionScreenState extends ConsumerState<TransactionScreen> {
									...
									child: ListView.builder(
										...
										itemBuilder: (context, index) {
											...
											return ListTile(
												...
												trailing: (!_baseExpenseCategories.contains(cat) && !_baseIncomeCategories.contains(cat) && cat != 'Uang Cash' && !ref.read(transactionProvider).wallets.any((w) => w.name == cat)) 
												? IconButton(
	...
	Widget build(BuildContext context) {
		...
		// --- FUNGSI BARU: MENGGABUNGKAN E-WALLET KE DALAM KATEGORI OTOMATIS ---
		final walletNames = state.wallets.map((w) => w.name).toList();
		
		List<String> currentCategories = isIncome
		? ['Uang Cash', ...walletNames, ..._baseIncomeCategories, ...state.customIncomeCategories]
		: ['Uang Cash', ...walletNames, ..._baseExpenseCategories, ...state.customExpenseCategories];
		...
		} else if (!currentCategories.contains(_selectedCategory)) {
			_selectedCategory = currentCategories.isNotEmpty ? currentCategories.first : 'Lainnya';
		}
							...
							children: [
								SizedBox(
									...
									child: SegmentedButton<String>(
										segments: const [
											ButtonSegment(value: 'expense', label: Text('Pengeluaran'), icon: Icon(Icons.arrow_upward_rounded)),
											ButtonSegment(value: 'income', label: Text('Pemasukan'), icon: Icon(Icons.arrow_downward_rounded)),
										],
										selected: {_selectedType},
										onSelectionChanged: (Set<String> newSelection) {
											setState(() {
												_selectedType = newSelection.first;
												_isCategoryInitialized = false; // Memaksa refresh kategori sesuai tipe baru
											});
										},
										style: SegmentedButton.styleFrom(selectedForegroundColor: Colors.white, selectedBackgroundColor: activeColor),
									),
								),
								...
								TextFormField(
									...
									decoration: InputDecoration(
										prefixText: 'Rp ', prefixStyle: theme.textTheme.headlineLarge?.copyWith(fontSize: 28, color: Colors.grey), hintText: '0',
										border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none),
										filled: true, fillColor: theme.brightness == Brightness.light ? const Color(0xFFF1F5F9) : const Color(0xFF1E293B),
								...
								TextFormField(
									...
									textCapitalization: TextCapitalization.words,
									decoration: InputDecoration(
										hintText: 'Contoh: Warteg Berkah', prefixIcon: const Icon(Icons.location_on_outlined),
										border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
										filled: true, fillColor: theme.brightness == Brightness.light ? const Color(0xFFF1F5F9) : const Color(0xFF1E293B),
									),
								),
													...
													InkWell(
														...
														child: Container(
															...
															child: Row(
																...
																children: [
																	Expanded(child: Text(_selectedCategory, style: theme.textTheme.bodyLarge, maxLines: 1, overflow: TextOverflow.ellipsis)),
													...
													InkWell(
														...
														child: Container(
															...
															child: Row(
																mainAxisAlignment: MainAxisAlignment.spaceBetween,
																children: [
																	Text(DateFormat('dd MMM').format(_selectedDate), style: theme.textTheme.bodyLarge),
								...
								if (_receiptImage != null)
									Stack(
										children: [
											ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.file(_receiptImage!, width: double.infinity, height: 200, fit: BoxFit.cover)),
											Positioned(
												top: 8, right: 8,
												child: CircleAvatar(
													backgroundColor: Colors.black54,
													child: IconButton(icon: const Icon(Icons.delete, color: Colors.white), onPressed: () => setState(() => _receiptImage = null)),
												),
											),
										],
									)
									...
									Row(
										children: [
											Expanded(
												child: OutlinedButton.icon(
													onPressed: () => _pickImage(ImageSource.camera), icon: const Icon(Icons.camera_alt_outlined), label: const Text('Kamera'),
											...
											Expanded(
												child: OutlinedButton.icon(
													onPressed: () => _pickImage(ImageSource.gallery), icon: const Icon(Icons.image_outlined), label: const Text('Galeri'),
													style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
												),
											),
										],
